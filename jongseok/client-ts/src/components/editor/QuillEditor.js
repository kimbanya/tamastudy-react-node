...
class ImageBlot extends BlockEmbed {
  static create(value) {
    ...
    imgTag.setAttribute('width', '100%');
    imgTag.setAttribute('min-height', '500px');
...
class VideoBlot extends BlockEmbed {
  static create(value) {
    if (value && value.src) {
      ...
      videoTag.setAttribute('minHeight', '500px');
      ...
      iframeTag.style.minHeight = '500px';
      return iframeTag;
    }
  }
...
class QuillEditor extends React.Component {
  ...
  insertImage = (e) => {
    ...
    if (e.currentTarget && e.currentTarget.files && e.currentTarget.files.length > 0) {
      ...
      API.post('/post/uploadpostfiles', formData, config).then((response) => {
        if (response.data.success) {
          ...
          quill.insertEmbed(position, 'image', {
            src: 'http://localhost:5000/' + response.data.url.split('public/')[1],
